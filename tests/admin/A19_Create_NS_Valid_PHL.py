...
class TestAdminCreateNSValidPHL(BaseTest):
    ...
    name = "A19. Create NS Valid Placement Handle List"
    ...
    def run(self, driver, log) -> TestResult:
        ...
        if create_res["rc"] == 0:
            ...
            # "Invalid Placement handle list" means nphndls conflicts with the
            # current device state (e.g. FDP already active with fixed handles).
            # This is a prerequisite issue, not a bug — skip gracefully.
            if "invalid placement handle" in err_lower or "handle list" in err_lower:
                log(f"⚠ Device rejected nphndls on create-ns: {stderr}")
                log("  This typically means the device requires a specific handle assignment")
                log("  that matches its active FDP configuration. Skipping.")
                return TestResult(TestStatus.SKIP,
                    f"create-ns with nphndls rejected by device ({stderr}). "
                    "Verify FDP configuration allows namespace creation with explicit handles.")
            log(f"✗ Namespace creation failed: {stderr}")
