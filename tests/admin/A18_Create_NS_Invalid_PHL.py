...
class TestAdminCreateNSInvalidPHL(BaseTest):
    ...
    name = "A18. Create NS Invalid Placement Handle List"
    ...
    def run(self, driver, log) -> TestResult:
        ...
        if create_res["rc"] != 0:
            ...
            rejection_strings = (
                "invalid field", "invalid format",
                "invalid placement handle", "invalid namespace",
            )
            if any(s in err_out for s in rejection_strings):
                log("✓ Command correctly rejected with an appropriate error.")
                ...
                return TestResult(TestStatus.WARN, f"Rejected, but error string not recognised. Got: {err_out}")
        else:
